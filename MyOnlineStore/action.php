...
if(isset($_POST["brand"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' class='brands' bid='$bid'>$brand_name</a></li>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["products"])){
	...
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
		<div class='col-md-4'>
		<figure class='snip1418'><img src='product_images/$pro_image'/>
		...
		</figcaption><a pid='$pro_id' style='float:right;' id='product'  ></a>
	  </figure>
	  </div>		
			";
		}
...
if(isset($_POST["categories"]) || isset($_POST["brands"])){
	...
	if (isset($_POST["getCartItem"])) {
		if (mysqli_num_rows($query) > 0) {
			...
			while ($row=mysqli_fetch_array($query)) {
				...
				<span class='item-price'>".CURRENCY." $product_price</span>
			  </li>
					";
				
			}
			?>
				<a href="cart.php" class="buttonfxx">Edit&nbsp;&nbsp;</a>
			<?php
			exit();
		}
	}
			...
			echo "<form method='post' action='login_form.php'>";
				...
				if (!isset($_SESSION["uid"])) {
					...
					echo '
						...
						<a class="buttonfxx_checkout_suc">"Check Out"</a>
						';
			}
		}
	}
